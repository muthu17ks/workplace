...
const Drive = () => {
    ...
    const handleOptionClick = (action) => {
        ...
        switch (action) {
            case 'create':
                ...
                break;
            case 'upload':
                document.querySelector('.drive-upload-input').click();
                break;
            case 'rename':
    ...
    const renderPath = () => {
        ...
        return (
            <>
                <span onClick={() => setCurrentFolder({ id: firstFolder.id, path: [firstFolder] })} className="drive-folder-path">
                    {firstFolder.name}
                </span>
                <span className="drive-folder-ellipsis">...</span>
    ...
    return (
        <div className='drive-body'>
            <div className="drive-container">
                ...
                <div classname="drive-heading-container">
                    <h1 className="drive-heading">Drive</h1>
                </div>
                <div className="drive-header">
                    <div className="drive-inner-header">
                        <span
                            className={`material-symbols-rounded drive-back-button ${currentFolder.path.length === 0 ? 'disabled' : ''}`}
                            onClick={currentFolder.path.length > 0 ? handleGoBack : null}
                            style={{ cursor: currentFolder.path.length === 0 ? 'not-allowed' : 'pointer' }}
                        >
                            arrow_back
                        </span>
                        <span className="material-symbols-rounded drive-refresh-button" onClick={() => loadFiles(currentFolder.id)}>refresh</span>
                        <div className="drive-search-bar">
                            <span className="material-symbols-rounded drive-search-icon">search</span>
                            <input
                                type="text"
                ...
                <div className="drive-path">
                    <span
                        className="drive-root-container"
                        onContextMenu={(e) => handleContextMenu(e, null)}
                    >
                        <span
                            onClick={() => setCurrentFolder({ id: 'root', path: [] })}
                            className="drive-root-button"
                        >
                            Drive
                        </span>
                    </span>
                ...
                <ul
                    ...
                    onContextMenu={(e) => {
                        if (e.target.closest('.drive-list-item') === null) {
                            handleContextMenu(e, null);
                        }
                    }}
                >
                    <li className="drive-list-header">
                        <span className="drive-list-header-name" onClick={() => handleSort('name')}>
                            Name
                            {sortField === 'name' && (
                        ...
                        <span className="drive-list-header-modified" onClick={() => handleSort('modified')}>
                            Date Modified
                            {sortField === 'modified' && (
                                <span className="material-symbols-rounded up-down-arrow">
                                    {sortOrder === 'asc' ? 'arrow_upward' : 'arrow_downward'}
                        ...
                        <span className="drive-list-header-size" onClick={() => handleSort('size')}>
                            Size
                            {sortField === 'size' && (
                                <span className="material-symbols-rounded up-down-arrow">
                                    {sortOrder === 'asc' ? 'arrow_upward' : 'arrow_downward'}
                    ...
                    {displayedFiles.map((file) => (
                        ...
                        >
                            <span className="drive-item-icon material-symbols-rounded">
                                {file.type === 'folder' ? 'folder' : 'description'}
                            </span>
                            <span className="drive-item-name">
                                {file.name}
                            </span>
                            <span className="drive-item-modified">
                ...
                />
                {isInputVisible && (
                    <div className="drive-new-folder-popup">
                        <button className="drive-close-button" onClick={handleClosePopup}><span className="material-symbols-rounded">close</span></button>
                        <input
                            type="text"
                            value={newFolderName}
                            ...
                            className="drive-new-folder-input"
                ...
                {showSharePopup && (
                    <div className="drive-share-popup">
                        <button className="drive-close-button" onClick={() => setShowSharePopup(false)}><span className="material-symbols-rounded">close</span></button>
                        <h3>Share</h3>
                        <input
                            type="text"
                            value={shareLink}
                            readOnly
                            className="drive-share-link-input"
                        />
                        <button
                            onClick={() => {
